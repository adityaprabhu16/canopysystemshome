const tag = "BR:08:M5:PI:13:OI:4Z:MR"; //FIXME: update based on tag!
//const tag = 0;
...
async function loadedHandler() {
    ...
    tuple = updateChart('Pressure (hPa)', 'hPa', pressure, pressureColor, 5, 0, 1400, 'pressurechart',pThresh);
    pressureChart = new Chart(tuple[0], tuple[1]);
    tuple = updateChart('Moisture (%)', '%', moisture, moistColor, 5, 0, 1200, 'moistchart',mThresh);
...
function processData(data) {
    ...
    for(let count = 0; count < totalEntries; count++) {
        ...
        let press = parseFloat(data[backIdx]?.['pressure'] ?? 0);
